...
const fetchPublicPresaleMigrateData = async(presaleType2Address: string): Promise<PublicPresaleData> => {
    ...
    const totalCakeLeft = new BigNumber(totalOnSale)
        .multipliedBy(new BigNumber(10).pow(18)) // presale amount uses 6 decimals
    ...
    return {
        ...
        BuyTokenPerCGS: BuyTokenPerCGS.toNumer()
    }
}

export default fetchPublicPresaleMigrateData