...
const ApyCalculatorModal: React.FC<ApyCalculatorModalProps> = ({
  ...
  const liquidityLink = getLiquidityExternalLink || getLiquidityLink(
    dex,
    !!isTokenOnly,
    tokenAddresses[process.env.REACT_APP_CHAIN_ID],
    cakeSymbol,
    quoteTokenAdresses[process.env.REACT_APP_CHAIN_ID],
    quoteTokenSymbol
  )


  // const farmApy = apy.times(new BigNumber(100)).toNumber()
  const farmApy = apy.toNumber()
  ...
  const cakeLabel = cakeSymbol || 'CGS'
  ...
  return (
    <Modal title="ROI" onDismiss={onDismiss}>
      <Grid>
        ...
        <GridItem>
          <Text fontSize="12px" bold color="textSubtle" textTransform="uppercase" mb="20px">
            {TranslateString(999, `${cakeLabel} per $1000`)}
          </Text>
        </GridItem>
        {/* 1 day row */}
        <GridItem>
      ...
      <Flex justifyContent="center">
        <LinkExternal href={liquidityLink}>
