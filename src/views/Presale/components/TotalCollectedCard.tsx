...
import { usePresalePublicData, useTotalCakeSold } from '../hooks/usePresalePublicData';
...
const HeadingText = styled(Heading)`
    ...
    color: ${({theme}) => theme.colors.bgLevel1}
`
...
const TotalCollectedCard : React.FC<TotalCollectedCardProps> = ({
    ...
    const optionData = usePresalePublicData(option)    
    
    const totalCakeSold = optionData.totalCakeSold ? 
        getBalanceAmount(new BigNumber(optionData.totalCakeSold), 6).toNumber() : 0

    const totalCollected = useMemo(() => {
        const totalColelected = optionData.totalCakeSold ? 
            getBalanceAmount(new BigNumber(optionData.totalCakeSold), 6).times(optionData.usdcPerCake) : BIG_ZERO;
